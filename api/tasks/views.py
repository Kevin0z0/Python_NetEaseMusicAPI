...
def task(request):
    ...
    if recommend_songs_resp.status_code != 200:
        logging.info(
            "status_code of recommend songs: " + str(recommend_songs_resp.status_code)
        )
    ...
    music_id: List = []
    for m in music_list:
        resp = send({"id": m, "n": 1000, "csrf_token": csrf}).POST(
            ...
            "logs": json.dumps(
                list(
                    map(
                        ...
                        random.sample(
                            music_id, 420 if len(music_id) > 420 else len(music_id)
                        ),
                    )
                )
            ),
